...
# TARGET_POS = [79.27, -58.94, -0.15] # Training loc 1
# TARGET_POS = [80.00, -97.22, -0.16] # Training loc 2
# TARGET_POS = [127.32, 25.28, -0.15] # Training loc 3
TARGET_POS = [117.51, -131.96, -0.15] # testing loc 1
MAX_HEIGHT = 10.0
...
class AirSimDroneEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if collision.has_collided:
            ...
            # ✅ NOW apply obstacle penalties
            seg_2d = pooled.reshape(5,5)

            front_obstacle = np.mean(seg_2d[0:2, 2:4])
            global_obstacle = np.mean(seg_2d)

            reward -= 3.0 * front_obstacle
            reward -= 1.0 * global_obstacle

            if front_obstacle > 0.3:
                reward += 0.5 * abs(vy)  # encourage lateral movement
